...
class MultiLinearExtension:
    ...
    def from_wiring_predicate(cls, wires, length, prime=2**61-1): 
        ...
        return cls([ Felt(1, prime) if i in ones else Felt(0, prime) for i in range(2**length) ], prime)
    ...
    def __add__(self, other):
        ...
        return self.__class__([ x + y for x,y in zip(self.evals, other.evals)], self.prime)
    ...
    def eval(self, point):
        def memo(r,n):
            ...
            return [ x for expr in memo(r,n-1) for x in [ expr * (Felt(1, self.prime) - r[n-1]), expr * r[n-1]]] 
