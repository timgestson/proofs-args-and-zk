...
class SumcheckProtocol:
    ...
    def __init__(self, prover: SumcheckProver, verifier: SumcheckVerifier, variables: int):
        self.prover = prover
    ...
    def poly_evals(self) -> list[Felt]:
        ...
        return [
            sum([
                Felt(1 - i) * a + Felt(i) * b 
                for (a,b) in 
                zip(self.evals[:half],self.evals[half:])
            ], Felt(0)) 
...
class SuperEfficientVerifier(SumcheckVerifier):
    ...
    def starting_round(self, c, evals) -> Felt:
        g1 = eval_from_points(evals, Felt(0)) \
           + eval_from_points(evals, Felt(1))
        assert(c == g1)
        r = Felt.random()
        self.rs.append(r)
    ...
    def execute_round(self, evals) -> Felt:
        gj = eval_from_points(evals, Felt(0)) \
           + eval_from_points(evals, Felt(1))
        assert(self.g_r == gj)
        r = Felt.random()
        self.rs.append(r)
